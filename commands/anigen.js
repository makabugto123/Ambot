...
module.exports = {
  ...
  name: 'anigen',  // Command name
  description: 'Generate HD anime picture through your prompt',  // Description
  usage: 'anigen [prompt]',  // Usage
  ...
  async execute(senderId, args, pageAccessToken) {
    ...
    const apiUrl = `https://hiroshi-api.onrender.com/ai/anime?prompt=${encodeURIComponent(prompt)}`;  // API endpoint with the prompt
const response = await axios.get(apiUrl);
const imglink = response.data.image_url;
    ...
    try {
      ...
      await sendMessage(senderId, {
        attachment: {
          type: 'image',
          payload: {
            url: imglink  // URL of the generated image
          }
        }
      }, pageAccessToken);
