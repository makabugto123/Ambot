...
module.exports = {
  ...
  name: 'confess',  // Command name
  description: 'generates an image based on a ID',  // Description
  usage: 'confess [uid]',  // Usage
  ...
  async execute(senderId, args, pageAccessToken) {
    ...
    if (!args || args.length === 0) {
      ...
      await sendMessage(senderId, {
        text: '❌ 𝗣𝗹𝗲𝗮𝘀𝗲 𝗽𝗿𝗼𝘃𝗶𝗱𝗲 𝘆𝗼𝘂𝗿 𝗽𝗿𝗼𝗺𝗽𝘁\n\n𝗘𝘅𝗮𝗺𝗽𝗹𝗲: confess 100007515468996.'
      }, pageAccessToken);
    ...
    const apiUrl = `https://api-canvass.vercel.app/confess?uid1=${senderId}&uid2=${encodeURIComponent(prompt)}`;  // API endpoint with the prompt
